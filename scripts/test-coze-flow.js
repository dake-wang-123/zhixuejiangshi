...
assert.strictEqual(extractNextStep('恭喜你完成第三关，现在进入第四关').nextStep, 0)
assert.strictEqual(extractNextStep('完成这一关，我们继续', 3).nextStep, 0)
assert.strictEqual(extractNextStep('你好呀，先自我介绍').nextStep, 0)
assert.strictEqual(extractNextStep('后面第8步会讲方法，现在先自我介绍').nextStep, 0)
assert.ok(startStepPrompt(4).indexOf('请开始第4步的内容') >= 0)
assert.strictEqual(stripGateMarkers('正文\n[下一关: 5]').indexOf('下一关') < 0, true)
const gated = applyProgress({ currentStep: 3, completedSteps: [1, 2] }, detectProgress('【步骤完成：3】\n[下一关: 4]'), 'reply')
...
assert.strictEqual(packedTurn, '我是讲师')
assert.ok(packedTurn.indexOf('current_step') < 0)
assert.ok(packedTurn.indexOf('请开始') < 0)
assert.strictEqual(packTurn('【进度上下文】\ncurrent_step=8\n---\n我的回答'), '我的回答')
...
const notDerailed = inferProgress([
...
const isolation = require('../miniprogram/utils/isolation.js')
isolation.resetBindsForTest()
assert.ok(isolation.switchedLesson('B01', 'C02'))
assert.ok(!isolation.switchedLesson('B01', 'B01'))
assert.ok(isolation.switchedLesson('', 'B01'))
assert.ok(isolation.switchedLesson('B01', ''))
assert.ok(!isolation.isCozeId('learn-user-B01'))
assert.ok(!isolation.isCozeId(''))
assert.strictEqual(isolation.conversationForLesson('B01', 'learn-user-B01'), '')
...
const turn = isolation.nextTurnId(livePage)
assert.ok(isolation.isLiveTurn(livePage, turn, 'lesson:B01'))
isolation.nextTurnId(livePage)
assert.ok(!isolation.isLiveTurn(livePage, turn, 'lesson:B01'))
livePage._turnId = turn
livePage.sessionKey = function () { return 'lesson:C02' }
assert.ok(!isolation.isLiveTurn(livePage, turn, 'lesson:B01'))
