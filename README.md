...
| 学习 | 已选课程的智学伴练进度。红格未完成，绿格已完成，可点格子回看。点「继续智学」回到该课 |
| 教案 | 上传文件到 `course.original_file`，粘贴全文后串联三个 ZAI，写回 `ai_analysis`；可把讲课稿生成课件 |
| 智学 | 未选课时从「选择课题」开始，允许智能体打招呼引导。选课学习走课程页，跳过招呼 |
...
- 点课程即进入智学伴练完整学习流程（选题 → 自我介绍 → … → 说课逐字稿）。从课程目录点进某课会跳过打招呼和选题确认，直接调取教案开始模块一。只有从「智学」Tab 未选课进入时，才触发智能体引导语。顶部 18 格红绿进度条对应各环节（对齐认知拆成收集/解释/心智三小块），点已完成格子可回看该环节对话。输入框支持按住麦克风转文字。进度写入 `study_record.progress`。
