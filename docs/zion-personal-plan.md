...
底部「教案」Tab 已下线。个人教案入口改到课程目录页最上方。提交后进入智学，走和系统课同一套 10 步交付法。个人教案正文放进 `user_message`，**不要**给「智学对话」行为流加新入参。
...
页面
├─ 顶栏 / 英雄区（选一门课开始学）
├─ 卡片：上传个人教案          ← 新增，和下面分组列表分开
│    ├─ 主标题：上传个人教案
│    ├─ 说明：上传你自己的教案，智能体会自动解析，并按照“十步交付法”引导你进行自学，产出你的专属讲课逐字稿。
│    ├─ 按钮：上传教案
│    └─ 展开后
