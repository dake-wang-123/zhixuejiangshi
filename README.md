...
| 智学 | 未选课时在此输入课题原题。学习页提交的课题也会跳到这里。环节完全跟 Coze 回复，不另编教学步骤 |
...
- 点已入库课程只把「开始学习《原题》」发给 Coze，学习环节以智能体回复列出的步骤为准，小程序不再编造模块一到说课稿。红绿进度条来自智学自己列出的环节，点已完成格子可回看。输入框支持按住麦克风转文字。进度写入 `study_record.progress`。
