...
自动发「请开始第 X 步」会把 Zion 的猜测塞给扣子，是脱轨主因，已经关掉。

过关后只用标记更新进度条，等讲师自己打字。不要给「智学对话」行为流加 `current_step` / `next_step` 入参，也不要写进 `user_message`。隔离见 `docs/zion-isolation.md`。
...
原生只保留 `extractNextStep` 更新进度条。`maybeAutoAdvance` 已删除，不再自动请求下一关。

只认 `【当前步骤：N】` / `【步骤完成：N】` / `[下一关: N]` / `【十步完成】`。正文里提到「第 8 步」不能跳步。

`current_step == 10` 且抽出 `[下一关: 11]` 或 `【十步完成】`：通关，解锁检验。
