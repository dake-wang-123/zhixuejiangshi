...
| Tab | 做什么 |
| --- | --- |
| 课程 | 打开后向智学 Coze 调取已上传的课程分类和课题原题，按智能体返回的分类展示。不本地编造分类。对不上的课题到学习页自行输入 |
...
- 课程目录打开时调用智学 Coze，列出知识库里已上传的分类和课题原题。分类名、课题名以智能体回复为准，不写死、不改写。下拉刷新会重新调取。对不上的课题到学习页自行输入原题。
- 学习环节只调用 Coze 智学。点已匹配课题只把**原题**发给智能体，用户回答原样回传，智学的问题原样展示。进度条只在智学自己列出环节后出现，不另编模块。「完成本环节」只把红格变绿，不向智学另发指令。输入框支持按住麦克风转文字。进度写入 `study_record.progress`。
...
| 环节 | 结果 |
| --- | --- |
| GraphQL 课程 / 分类 | 通。目录分类改由智学 Coze 现场返回；Zion `course` 只用来标「已入库」。对不上智学原题的课到学习页自行输入。 |
