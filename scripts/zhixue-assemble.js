...
function arg(name, fallback) {
  try {
    var value = context.getArg(name);
    if (value === 0 || value) return value;
  } catch (e) {}
  return fallback === undefined ? '' : fallback;
}

var rawBot = arg('bot_id', '');
var botId = (rawBot === 0 || rawBot) ? String(rawBot) : fallbackBotId;
if (!/^\d{10,}$/.test(botId)) {
  botId = fallbackBotId;
}
var userId = String(arg('user_id', 'lecturer') || 'lecturer');
var message = String(arg('user_message', '') || '');
var conv = String(arg('conversation_id', '') || '');
...
var auth = normalizeAuth(arg('api_key', ''));
var tpaCode = 0;
var tpaMsg = '';
var chatId = '';
...
if (message.indexOf('__POLL_CHAT__|') === 0) {
  ...
  var additional = [{ role: 'user', content: message, content_type: 'text' }];
  var fzBody = {
    bot_id: botId,
    user_id: userId,
    stream: false,
